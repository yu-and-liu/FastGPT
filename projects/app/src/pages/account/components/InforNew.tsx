...
import type { UserType, standardInfoType } from '@fastgpt/global/support/user/type.d';
import { standardInfoEnum, UserStandardEnum } from '@fastgpt/global/support/user/constant';
...
const UserInfo = () => {
  ...
  const [standardInfo, setStandardInfo] = useState<standardInfoType | undefined>(undefined);
  ...
  const {
    ...
    const { standard = {}, datasetMaxSize = 0, standardMaxDatasetSize = 0, standardMaxPoints = 0, totalPoints = 0, usedDatasetSize = 0, usedPoints = 0 } = res;
    console.log('res', res);
    setStandardInfo({
    ...
    <Box m={[24, 16]}>
      ...
      <Flex
        ...
        <Flex ml='8' justifyContent={'space-between'} w={'100%'}>
          <Box w={'50%'}>
            {feConfigs.isPlus && (
              <Flex mb={4} alignItems={'center'} w={'100%'}>
                <Box flex={'0 0 80px'} as='b'>{t('user.Member Name')}:&nbsp;</Box>
                <Input
                  flex={1}
            ...
            <Flex alignItems={'center'} w={'90%'}>
              <Box flex={'0 0 80px'} as='b'>{t('user.Account')}:&nbsp;</Box>
              <Box flex={1}>{userInfo?.username}</Box>
            </Flex>
            <Flex mt={3.5} alignItems={'center'} w={'80%'}>
              <Box flex={'0 0 80px'} as='b'>{t('user.Password')}:&nbsp;</Box>
              <Box flex={1}>*****</Box>
          ...
          <Box w={'50%'}>
            <Flex alignItems={'center'} w={'90%'}>
              <Box flex={'0 0 80px'} as='b'>{t('user.Team')}:&nbsp;</Box>
              <Box flex={1}>
                <TeamMenu />
              </Box>
            </Flex>
      ...
      <Flex
        ...
        >
          <Stack
            ...
            borderColor={'E8EBF0'}>
            <Text color={'#485264'} fontSize='l'>当前套餐</Text>
            <Text color={'#485264'} fontSize='xl' as='b'>{'免费套餐'}</Text>
            <Text mt='4' color={'#485264'} fontSize='m'>{'过期时间：  ' + standardInfo?.expiredTime}</Text>
            <Button mt='7' colorScheme='blue'>升级套餐</Button>
          </Stack>
          <Stack
            bg={'white'}
            overflow={'scroll'}
            ...
            <Flex alignItems={'center'}>
              ...
              {standardInfoEnum[standardInfo?.currentSubLevel]?.maxTeamNum + '个成员'}
            </Flex>
            <Flex mt='1' alignItems={'center'}>
              <MyIcon mr={2} name={'acount/check'} w={'20px'} />
              {standardInfoEnum[standardInfo?.currentSubLevel]?.maxAppNum + '个应用插件'}
            </Flex>
            <Flex mt='1' alignItems={'center'}>
              <MyIcon mr={2} name={'acount/check'} w={'20px'} />
            ...
            <Flex mt='1' alignItems={'center'}>
              <MyIcon mr={2} name={'acount/check'} w={'20px'} />
              {standardInfoEnum[standardInfo?.currentSubLevel]?.maxPreservation + '天历史记录保持'}
            </Flex>
            <Flex mt='1' alignItems={'center'}>
              <MyIcon mr={2} name={'acount/check'} w={'20px'} />
              {standardInfo?.datasetMaxSize + '个向量存储'}
            </Flex>
            {standardInfoEnum[standardInfo?.currentSubLevel]?.other.map((item: string) => {
              return (
                <Flex mt='1' alignItems={'center'}>
                  <MyIcon mr={2} name={'acount/check'} w={'20px'} />
                  {item}
        ...
        <Box
          ...
          <Box width={'100%'}>
            <Flex alignItems={'center'} justifyContent={'space-between'} width={'100%'}>
              <Flex alignItems={'center'} as='b' mr='3' fontSize='2xl'>知识库存储
              ...
              <Flex
                alignItems={'center'}>
                <Text color={'#2B5FD9'} fontWeight={'500'} mr='2' fontSize='l'>购买额外存储</Text>
            ...
            <Flex alignItems={'center'} justifyContent={'space-between'} width={'100%'}>
              <Flex alignItems={'center'} as='b' fontSize='2xl'>
                ...
                <Text color='#485264' fontWeight='500' ml='3' fontSize='l'>
                  {standardInfo?.usedDatasetSize + " / " + standardInfo?.standardMaxPoints + '  积分'}
                </Text>
              </Flex>
              <Flex
                alignItems={'center'}>
                <Text color={'#2B5FD9'} fontWeight='500' mr='2' fontSize='l'>购买额外积分</Text>
                <MyIcon name={'acount/arrowRight'} w={'20px'} />
              </Flex>
